...
class Learner():
	...
	def __init__(self , test):
		...
		self.scores = np.zeros(shape = self.no_of_problems)
		self.decay_rates = np.zeros(shape = self.no_of_problems) 
		self.times = np.zeros(shape = self.no_of_problems)
	...
	def decay_scores(self):
		...
		for i in range(self.no_of_problems):
			if self.scores[i] == 0:
				...
				self.scores[i] = self.scores[i] * np.exp(-1*self.decay_rates)[i]
	...
	def show_overall_score_graph(self):
		"""
	...
	def next_question(self):
		...
		if self.latest_question_index >= self.no_of_problems - 1 and len(self.review_queue) == 0:
			...
			# for i in range(self.latest_question_index):
			# 	if self.problems_list[i] == prob:
			# 		self.curr_question_index = i
	...
	def answer(self , problem , ans , time):
		...
		Parameters:
		problem : Problem which us answered
		ans     : Response given
		time    : Response time
		"""
	...
	def show_question_graph(self , index):
		"""
